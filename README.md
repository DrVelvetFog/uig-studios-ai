...
- **Safety gates that don't depend on the model.** Hard denylist of catastrophic operations, approval prompt for anything mutating (with an allowlist for "always allow"), prompt-injection scanning on all web/MCP content, and a provenance flag: once untrusted content enters the context, mutating tools require human approval for the rest of the turn regardless of allowlists. Secrets live in `~/.tonyai/secret-*.txt` (mode 0600), never in the webview.
