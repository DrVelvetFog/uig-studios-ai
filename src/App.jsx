...
function DevInspectPanel({ accent, onClose }) {
...
// ── Inbox Panel ───────────────────────────────────────────────────────────────
function InboxPanel({ inbox, onMarkRead, onMarkAllRead, onAsk, onClose, isDark }) {
...
const RUNNABLE_LANGS = new Set(["python","python3","py","javascript","js","typescript","ts","bash","sh","shell","zsh"]);

function CodeBlock({ code, lang, onRun }) {
...
function ThinkBlock({ text, streaming = false }) {
...
function renderCodeSegment(text, keyPrefix, onRun) {
...
function renderMessage(text, onRun) {
  const result = [];
...
const CODE_EXTS_SET = new Set([".py",".js",".ts",".jsx",".tsx",".sh",".rb",".go",".rs",".java",".c",".cpp",".mjs"]);
...
function validateToolArgs(fnName, fnArgs, allTools) {
...
function enrichToolError(fnName, rawError) {
...
const CURRENT_INFO_RE = /\b(?:current|latest|now|today|right now|price|status|recent|news|update|this week|2024|2025|2026|live|real.?time)\b/i;
function neededSearchButSkipped(userPrompt, toolSteps) {
...
function evaluateStopCondition(loopToolSteps) {
